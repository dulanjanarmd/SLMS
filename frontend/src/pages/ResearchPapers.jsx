              ...
              {papers.map(p => (
                <div key={p.id} style={{ background: 'white', borderRadius: 16, overflow: 'hidden', border: '1px solid #e8ecf0', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', transition: 'all 0.2s' }}
                  ...
                  <div style={{ padding: p.coverImageUrl ? '18px 22px 22px' : '14px 22px 22px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    {p.researchField && <span style={{ display: 'inline-block', width: 'fit-content', background: 'rgba(99,102,241,0.1)', color: '#4f46e5', padding: '3px 10px', borderRadius: 6, fontSize: '0.72rem', fontWeight: 600, marginBottom: 10 }}>🏛️ {p.researchField}</span>}
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => handleView(p)} style={{ flex: 1, background: 'linear-gradient(135deg, #ef5a24, #ff8c5a)', color: 'white', border: 'none', borderRadius: 10, padding: '10px', fontWeight: 700, fontSize: '0.82rem', cursor: 'pointer', fontFamily: 'Poppins, sans-serif', boxShadow: '0 4px 12px rgba(239,90,36,0.22)' }}>👁️ Read Online</button>
                      <button onClick={() => handleDownload(p)} style={{ flex: 1, background: 'linear-gradient(135deg, #1e3a8a, #06b6d4)', color: 'white', border: 'none', borderRadius: 10, padding: '10px', fontWeight: 700, fontSize: '0.82rem', cursor: 'pointer', fontFamily: 'Poppins, sans-serif' }}>⬇️ Download</button>
                      {isLibrarian && <button onClick={() => handleDelete(p.id)} style={{ padding: '0 14px', background: 'rgba(239,68,68,0.08)', color: '#ef4444', border: '1.5px solid rgba(239,68,68,0.2)', borderRadius: 10, fontWeight: 700, cursor: 'pointer' }}>🗑️</button>}
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', flexDirection: 'column', zIndex: 10000, padding: 20 }} onClick={e => { if (e.target === e.currentTarget) closeViewer(); }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', color: 'white', marginBottom: 12, fontFamily: 'Poppins, sans-serif' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => handleDownload(viewing)} style={{ padding: '9px 18px', background: 'linear-gradient(135deg, #1e3a8a, #06b6d4)', color: 'white', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', fontFamily: 'Poppins, sans-serif' }}>⬇️ Download</button>
